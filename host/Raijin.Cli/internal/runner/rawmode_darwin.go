...
// on the output side. term.MakeRaw on macOS clears OPOST, which disables
...
//
// macOS uses TIOCGETA/TIOCSETA for the get/set termios ioctls (BSD lineage),
// where Linux uses TCGETS/TCSETS. The flag set is otherwise identical.
func enterRawMode(fd int) (restore func() error, err error) {
	...
	n.Cc[unix.VMIN] = 1
	n.Cc[unix.VTIME] = 0
	if err := unix.IoctlSetTermios(fd, unix.TIOCSETA, &n); err != nil {
		return nil, err
	}
	return func() error { return unix.IoctlSetTermios(fd, unix.TIOCSETA, old) }, nil
}
